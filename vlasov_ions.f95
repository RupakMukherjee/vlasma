...
real*8 L, dt, x, dx, kx, CFL, dt_new
...
vi = Vimin + dfloat(j-1) * dvi
  do i = 1,N
  x = dfloat(i-1) * dx
  fex(i) = 1.0d0+0.05d0*dcos(2.0d0*pi*x/L)
  fix(i) = 1.0d0
  fev(j) = (1.0d0 / dsqrt(2.0d0*pi)) * dexp( -ve*ve / (2.0d0*Veth*Veth) )
  fiv(j) = (1.0d0 / dsqrt(2.0d0*pi)) * dexp( -vi*vi / (2.0d0*Vith*Vith) )
...
if (t .ge. 900 .and. mod(t,1000) == 0) then
  do i = 1,N
    ...
    do j = 1,M
      ve = Vemin + dfloat(j-1) * dve
      vi = Vimin + dfloat(j-1) * dvi
      write(s,*) x,ve, vi, fexv(i,j), fixv(i,j)
    enddo
  enddo
  s = s+1
endif
